...
try:
    ...
    hold_client = InfluxDBClient(INFLUXDB_HOST, INFLUXDB_PORT, INFLUXDB_USER, INFLUXDB_PASSWORD, HOLD_DBNAME)
...
def store_failed_reading(json_object):
    """Store failed reading in Hold DB"""
    try:
        point = {
...
def send_json_to_server(json_object):
    ...
    try:
        ...
        if response_json.get("responseCode") == 200:
            print("✅ Data successfully sent, no need to store in Hold DB")
            return True
        else:
...
def fetch_latest_sensor_data(sensor_id):
    ...
    try:
        ...
        temperature = next(temp_result.get_points(), {}).get("temperature")
        humidity = next(humidity_result.get_points(), {}).get("humidity")
...
def listen_for_new_data():
    """Listen for sensor updates and store each reading"""
    print("🔄 Listening for new sensor updates...")

    while True:
        retry_failed_readings()  # Try resending failed readings
        ...
        for sensor_id in SENSOR_IDS:
            ...
            # ✅ Save to Hold DB & Send to Server
            if not send_json_to_server(json_object):
                store_failed_reading(json_object)

        time.sleep(10)  # Adjust frequency to match actual sensor update rate
